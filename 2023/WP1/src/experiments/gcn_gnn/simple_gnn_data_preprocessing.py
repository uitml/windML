...
def generate_knn_embedding(distance_df: pd.DataFrame,
    ...
    for t in range(1, t):
        ...
        coo_emb.data = np.hstack((coo_emb.data,
                                  np.zeros_like(skiprow) + temporal_strength,
                                  orgdata
                                  ))
...
def frost_api_to_gnn_dataset(data_df: pd.DataFrame,
    ...
    temporal_knn, station_order = generate_knn_embedding(distance_df, timesteps, station_ids, k, temporal_strength)
...
if __name__ == '__main__': 
    ...
    frost_api_to_gnn_dataset(data, adj_df, 'wind_speed', n_skips=3, multiedge=True)
    #print(emb.row.shape, emb.col.shape, emb.data.shape)
